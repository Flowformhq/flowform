...
test('POST /api/v1/submissions creates a submission anonymously for an active form', function () {
...
test('POST /api/v1/submissions returns 422 for an inactive form', function () {
    $form = Form::create(['name' => 'Inactive Form', 'is_active' => false]);
...
test('POST /api/v1/submissions returns 422 for unknown form_uuid', function () {
    $this->postJson('/api/v1/submissions', ['form_uuid' => 'does-not-exist'])
        ->assertUnprocessable();
});

test('GET /api/v1/submissions/{uuid} returns submission with values map anonymously', function () {
    $form = Form::create(['name' => 'My Form', 'is_active' => true]);
...
test('PATCH /api/v1/submissions/{uuid} updates status anonymously', function () {
    $form = Form::create(['name' => 'My Form', 'is_active' => true]);
    Step::create(['form_id' => $form->id, 'step_number' => 1, 'title' => 'Step 1']);
    $submission = Submission::create(['form_id' => $form->id]);

    $this->patchJson("/api/v1/submissions/{$submission->uuid}", ['status' => 'completed'])
        ->assertOk()
        ->assertJsonPath('data.status', 'completed');
});

test('POST /api/v1/submissions/{uuid}/values upserts field values anonymously', function () {
    $form = Form::create(['name' => 'My Form', 'is_active' => true]);
    $step = Step::create(['form_id' => $form->id, 'step_number' => 1, 'title' => 'Step 1']);
    $ft = FieldType::create(['name' => 'text', 'component' => 'text-input']);
    Field::create([
    ...
    $this->postJson("/api/v1/submissions/{$submission->uuid}/values", [
        'values' => [
...
test('POST /api/v1/submissions/{uuid}/advance advances the step anonymously', function () {
    ...
    $this->postJson("/api/v1/submissions/{$submission->uuid}/advance")
        ->assertOk()
...
test('POST /api/v1/submissions/{uuid}/retreat retreats the step anonymously', function () {
    $form = Form::create(['name' => 'Multi-step Form', 'is_active' => true]);
    Step::create(['form_id' => $form->id, 'step_number' => 1, 'title' => 'Step 1']);
    Step::create(['form_id' => $form->id, 'step_number' => 2, 'title' => 'Step 2']);
    $submission = Submission::create(['form_id' => $form->id, 'current_step' => 2]);

    $this->postJson("/api/v1/submissions/{$submission->uuid}/retreat")
        ->assertOk()
        ->assertJsonPath('current_step', 1);
});

test('GET /api/v1/submissions/{uuid}/conditions returns field states anonymously', function () {
    ...
    $this->getJson("/api/v1/submissions/{$submission->uuid}/conditions")
        ->assertOk()
